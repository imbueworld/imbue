...
export function representDatabaseField(field) {
  switch (field) {
    ...
    default:
      if (config.DEBUG)
        console.warn(
          `HelperFunctions/representDatabaseField field ${field} not set.`,
        );
      return field;
  }
}
...
export async function requestPermissions(perms) {
  if ((Platform.OS = 'android')) {
    const PermissionObjects = perms.map(
      (permName) => PermissionsAndroid.PERMISSIONS[permName],
    );
...
export async function logOutAndRedirect(navigation) {
  if (!navigation) {
    if (config.DEBUG)
      console.warn(
        'logOutAndRedirect did not execute because param `navigation` was not provided, it should be something you get from useNavigation().',
      );
    return;
  }

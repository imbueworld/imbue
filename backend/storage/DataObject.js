...
export default class DataObject {
  ...
  async push(options = {}) {
    ...
    if (!this.uid && !forceNew) {
      if (config.DEBUG) {
        console.warn(
          'Push was not done, manual pushing is not permitted. Use or create method: .create() for the respective Class (Gym, Class etc.)',
        );
        this.__DEBUG();
      }
      return;
    ...
    for (let item in modifiedData) {
      ...
      if (structure[item] !== new Object(modifiedData[item]).constructor) {
        if (config.DEBUG)
          console.warn(
            `Item '${item}' (${
              new Object(modifiedData[item]).constructor.name
            }: ${
              modifiedData[item]
            }) was not pushed to database, because it was not of required type: ${
              structure[item].name
            }`,
          );
        continue;
    ...
    let firebaseDoc = await this._getDocDbRef().set(dataToBePushed, {
      merge: true,
    });
    this._modifiedFields = []; // fields have been synced
  ...
  async delete() {
    await this._getDocDbRef().set(null);
  }

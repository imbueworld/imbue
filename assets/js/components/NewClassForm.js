...
import { createClass } from '../backend/BackendFunctions'
...
export default function NewClassForm(props) {
    ...
    useEffect(() => {
        const init = async() => {
            let newClassForm = snatchNewClassForm(cache)
            ...
            let associatedGyms = ( await retrieveUserData(cache) ).associated_gyms
            let gyms = await retrieveGymsByIds(cache, { gymIds: associatedGyms })
    ...
    useEffect(() => {
        let form = cache.temp.newClassForm

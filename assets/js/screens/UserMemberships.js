...
export default function UserMemberships(props) {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const gymDocs = (
        await gyms.retrieveWhere('id', 'in', active_memberships)
      ).map((it) => it.getAll());
    ...
    MembershipsCreate(
      memberships.map((membership) => (
        <TouchableMenu
          ...
          <ActiveMembershipBadge data={membership} key={membership.id} />
        </TouchableMenu>
      )),
    );
    ...
    <>
      ...
      <ProfileLayout>
          ...
          onPress={async () => {
            ...
            PastTransactionsCreate(
              pastTransactions.map((transaction, idx) => (
                <TransactionView data={transaction} key={idx} />
              )),
            );
          }}
        />
        ...
        <Text
          ...
          In order to cancel a membership, tap and hold it's respective card and
          tap remove.
        </Text>
      </ProfileLayout>
    </>

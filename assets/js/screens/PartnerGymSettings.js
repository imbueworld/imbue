...
export default function PartnerGymSettings(props) {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const { associated_gyms = [] } = userDoc;

      const gym = new Gym();
      const gymDoc = (await gym.retrieveGym(associated_gyms[0])) || {};
      setGym(gymDoc);

      const { name, description, formatted_address } = gymDoc;
  ...
  return (
    <ProfileLayout
      ...
      <CustomButton
        icon={
          <Icon source={require('../../../components/img/png/ellipsis.png')} />
        }
        title="Revenue Information"
        onPress={() => props.navigation.navigate('PartnerRevenueInfo')}
      />
      ...
      <CustomButton
        ...
        onPress={() => props.navigation.navigate('PartnerUpdateMemberships')}
      />
      <CustomButton
        icon={
          <Icon source={require('../../../components/img/png/ellipsis.png')} />

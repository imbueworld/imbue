...
export default function PurchaseUnlimited(props) {
  ...
  useEffect(() => {
    ...
    setHasImbueMembership(user.active_memberships.includes(imbueMembership.id));
  }, [activeMembershipsCount, imbueMembership, popup]);

  if (!imbueMembership) return <View />;

  // helper variable
  const membershipPrice = `$${currencyFromZeroDecimal(
    imbueMembership.membership_price,
  )}`;
